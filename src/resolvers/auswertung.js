  ...
  Query: {
    auswertungSchueler: async (obj, { id }, { db, permission }) => {
      ...
      const [rows] = await db.query(
        'SELECT s.vorname, s.nachname, s.geschlecht, k.stufe, k.name FROM schueler s LEFT JOIN klassen k ON k.id = s.idklasse WHERE s.id = ?',
        [id],
      );
        ...
        for (let i = 0; i < massstaebe.length; i += 1) {
          if (disziplin.best === 'high') {
            if (e.wert > massstaebe[i].werte) {
              return { wert: e.wert, punkte: massstaebe[i - 1].punkte, disziplin: e.iddisziplin };
            }
      ...
      const summePunkte = res.reduce((a, b) => a.punkte + b.punkte, { punkte: 0 });
